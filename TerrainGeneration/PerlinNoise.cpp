...
double PerlinNoise::perlin(double x, double y, double z)
{
	...
	double x1, x2, y1, y2;
	x1 = lerp(grad(aa, xf, yf, 0),				// The gradient function calculates the dot product between a pseudorandom
		grad(ba, xf - 1, yf, 0),				// gradient vector and the vector from the input coordinate to the 8
		u);										// surrounding points in its unit cube.
	x2 = lerp(grad(ab, xf, yf - 1, 0),				// This is all then lerped together as a sort of weighted average based on the faded (u,v,w)
		grad(bb, xf - 1, yf - 1, 0),				// values we made earlier.
		u);
